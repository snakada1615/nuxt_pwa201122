    ...
    methods: {
      async setWorkspace(val, flag) {
        ...
        try {
          ...
          await vm.$store.dispatch('saveUserToLastuser', {user: vm.$store.state.user, caseId: val})

          console.log('setWorkspace02')
          if (flag === 2) {
            //initialieze user workspace
            await vm.$store.dispatch('initPouch', {
              user: vm.$store.state.user, caseId: val, fctDb: vm.$store.state.fctDb
            })
          } else {
